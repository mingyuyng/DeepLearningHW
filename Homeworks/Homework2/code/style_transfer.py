...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    loss = 0
    for count, layers in enumerate(style_layers):
        loss += style_weights[count] * torch.sum((gram_matrix(feats[layers]) - style_targets[count])**2)
...
def style_transfer(content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        # TODO:Compute loss
        content_current = feats[content_layer].clone()

        c_loss = content_loss(content_weight, content_current, content_target)
